...
fnameBUFR=${DIR_DATA}/BUFR-temp-${DATE}.txt
...
for ecenter in ${centro};do
  ...
  for file_BUFR in $(ls "${DIR_DATA}"/BUFR_temp_"${ecenter}"/*_"${ecenter}"_*00.bufr );do
    ...
    for isubset in $(seq 1 "${nsubset}");do
      ...
      bufr_filter filter_file ${file_BUFR} | grep "PREC" | grep -v "MISSING" | awk -F',' '{if ($8>0) print $0}' >> filter.out.prec
...
exit
  ...
  mv filter.out.station ${DIR_DATA}/BUFR-STATION-$ecenter.csv

   #FIXME ñapa para evitar estaciones repetidas
  touch ${DIR_DATA}/BUFR-STATION-sinrepetir.csv

  while IFS=, read -r station station_number lon lat alt name
  do
    rep_station=$(cat ${DIR_DATA}/BUFR-STATION-sinrepetir.csv | awk '{if ($2=='${station_number}') print 1}')
    if [[ "${rep_station}" -ne 1 ]]; then
      echo $ecenter ${station_number} $lon $lat $alt $name >> ${DIR_DATA}/BUFR-STATION-sinrepetir.csv
    fi
  done < ${DIR_DATA}/BUFR-STATION-$ecenter.csv
...
done < ${DIR_DATA}/BUFR-STATION-sinrepetir.csv
